...
通过上面的例子，我们可以看到代码和渲染的效果。如果有“命令和环境”的使用示例，那么LATEX包的文档就更容易读懂。最好的方法是对\LaTeX{}代码和格式化输出进行比较。

showexpl 是一个用于进行比较的包，它基于包列表， 该列表提供了良好的排版源代码， 强调了关键字等等。
...
当然，大家看到的本文很多代码是双边框的。为什么呢？ 因为有一个设置是 frame ， 有效值是 lrtb 和 LRTB。 大写则为双边框。大家还记得 \ref{sec:command} 吗？ 可选参数是如何配置的？可选参数是[] 方括号，即 \verb!\begin{lstlisting}!后面可以写[frame = r/l/t/b/L/R/T/B]。
