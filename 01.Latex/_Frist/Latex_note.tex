...
很明显，\LaTeX 把很多内容当作一个 \fbox{ Box } 进行显示，也是就是划定一个区域（Box）出来，你可以在这个区域里面进行绘制， 但是里面如何绘制是这个盒子的事情，那么这个盒子有要放在哪里呢？

我们可以图片和表格等内容当作为一个Box，那么我们就可以按照 \LaTeX 对盒子的美学排版进行设置这个盒子的配置。 看表 \ref{tab:position_of_box}，你会很好奇，为什么这个表跑到上面去了，因为我在绘制表的时候，没有设置参数，那么就是默认的[tbp]，也就是顶部，所以他报道顶部了。
...
默认情况下是 tbp, 同时需要注意的是，\LaTeX 总是尝试以 h-t-b-p 的顺序来确定图形的位置。 当然，加一个！ 可以强制按照指定的顺序来执行。 如!bpt 则会先考虑b，在考虑p，之后考虑t， 而不默认的htbp。
