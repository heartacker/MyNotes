...
参考：\href{https://yuxtech.club/2019/07/30/listings/}{解决listing 嵌套的问题}

\paragraph{实现原理}

如果使用两个lstlisting环境嵌套，会使得第一个\verb!\begin{lstlisting}!匹配到第一个\verb!\end{lstlisting}!而使得第二个报错。比较不错的解决方法就是把外层的摘录环境换成verbatim。

\sout{不过由于个人比较喜欢listings包的风格，所以}这里采用tcolorbox包的listings库提供的tcblisting环境。tcblisting环境其实就是调用的listings包，它一方面接受listings包的设置，另一方面还接受tcolorbox包的设置，简直一举两得。默认情况下，摘录的同时还执行摘录的代码，所以这里添加了listing only选项，仅仅作摘录使用。 \footnote{https://yuxtech.club/2019/07/30/listings/}
...
\subparagraph{使用 tcblisting 包围}代码:
...
\end{tcblisting}
\end{verbatim}
这里需要注意的是，一定要使用{\bfseries listling only} 参数。 同事为了防止边框重叠。添加一些边框参数。总体要添加{\rmfamily \{listing only,boxrule=-1pt,colback=white\}}

显示效果参考\ref{lst:tcbdemo}
