...
\begin{center}
    \begin{figure}[H]
        ...
        \fbox{
            \includegraphics[scale=0.6]{images/article_render.png}
        }
...
\begin{center}
    \begin{figure}[H]
        ...
        \setlength{\fboxsep}{0cm}
        \fbox{
            \includegraphics[scale=0.6]{images/ctexart_render.png}
        }
...
1. 引用宏包，这样会默认在链接上显示红框：\verb!\usepackage{hyperref}!

2. 引用宏包，这样会在链接上字体显示为红色\verb!\usepackage[colorlinks=true]{hyperref}!
...
1. label 图表建议添加label到环境的后面。否则标号不对。
...
通过上面的例子，我们可以看到代码和渲染的效果。如果有 \LaTeX “命令和环境”的使用示例，那么 \LaTeX 宏包的文档就更容易读懂。最好的方法是对 \LaTeX 代码和格式化输出进行比较。
...
pos 是可选参数。参数为 lrtb，分别代表 l左边， r右边， t顶部和 b底部。
