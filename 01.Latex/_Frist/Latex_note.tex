...
无论是你在哪里看到我做个文档，其实我不是跟你在教学 \LaTeX ，我不是专业的 \LaTeX 高手，我这个文档也不是给专业的用户而写。

在作为一个门外汉，我在接触 Markdown 写公式之后，感受到 \LaTeX 对数学的强大的支持，就觉得有必要学习这个专业的排版系统，感受他的魅力。因此从零开始折腾，从入门知识，到开始写文档，添加各种内容，使用各种环境，以及对细节的调整和把控。 我这里只是给你一个最简单最基础的入门。 让你能做出最简单的文档，让你能够在学习过程中，知道该如何入门学习，该如何去找资料看，该让环境工作起来，让你能够提笔写东西的时候，能生成一个最基础的文档。而不是这里查查，那儿看看。最后还是不能让环境编译起来。甚至不知道该如何下手。

我这里的基础知识很多东西都是非常简陋的，我是摘抄自 海洋老师的书籍 \cite{latex_start}。一来是我本来不是很专业的作者，而来是我就是为了入门才学习这个东西的，我也是为了入门，才记录我的陡峭的学习过程和学习遇到的困难。 但你在看到我这个文档时候， 我已经完成了蜕变。 恭喜我自己，也恭喜你，我们一直在进步。
...
一个 \LaTeX 换进自然就是一个分组（Group），因此，前面的 \verb!\zihao{-5}! 会影响 quote 换进。其中，最大分组就是 document 了。不过也可以用 \{\ \} 产生一个分组。
...
\subsubsection{列表公式}
较长的公式或者大块公式，一般单独居中编写，为了方便引用，还要进行编号。我们一般叫 $displayed formula$。使用 equation 环境方便输出。参考 \href{https://matnoble.me/tech/latex/multi-line-equations/}{各种样子的对齐编码}。
...
\href{https://www.cnblogs.com/yifdu25/p/8330911.html}{参考一} 。
...
第一步，在在工具链添加这个工具（latex-workshop.latex.tools），第二步中会调用这个命令。
\begin{lstlisting}    
{
    "name": "xelatex",
    "command": "xelatex",
...
\begin{quote}
    \zihao{5}\kaishu \textit{实现}：在文中的意思就是指“程序”的意思。如文中：eTeX 程序和 Knuth TeX 都是TeX语言的一个实现（也就是说，eTeX 程序和 Knuth TeX 都是把TeX语言转换为排版的程序。程序作用于tex文本文件，把tex文件编译成dvi文件）。
\end{quote}
